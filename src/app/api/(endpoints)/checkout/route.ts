...
async function handleCartCheckout(data: any) {
  ...
  for (const item of cart.items) {
    ...
    if (grocery.stock < item.quantity) {
      return NextResponse.json(
        {
          message: `Insufficient stock: ${item.name} - Available: ${grocery.stock}, Requested: ${item.quantity}`,
        },
        { status: 400 }
      );
    }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    mode: "payment",
    success_url: (process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000") + "/success?userId=" + userId,
    cancel_url: (process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000") + "/cancel?userId=" + userId,
  });
